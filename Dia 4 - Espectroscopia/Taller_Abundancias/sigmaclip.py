...
def clip(data, mean, sigma, siglow, sighigh, indices=None):
    ...
    if indices is not None:
        ilow = np.logical_and(data >= mean - sigma * siglow, indices)
        ihigh = np.logical_and(data <= mean + sigma * sighigh, indices)
    else:
...
def calcmean(data, errors=None):
    ...
    if errors is None:
        mean = data.sum() / N
        sigma = np.sqrt(((data**2).sum() - N * mean**2) / (N - 1) / N)
        sigint=0
    else:
        w = 1. / errors**2
        mean = (w * data).sum() / w.sum()
        sigma = w.sum()/((w.sum())**2-(w**2).sum())*(w*(data-mean)**2).sum()
        sigint = (w*errors).sum()/w.sum()
    return mean, sigma, sigint, N
...
def calcsigma(data, errors=None, mean=None, axis=None, errors_as_weight=False):
    ...
    if w is not None:
        ...
        if axis:
            ...
            shape.insert(axis, 1)
            mmean = np.array(mean, copy=0, ndmin=data.ndim).reshape(shape)
        else:
            mmean = mean
        sigma = np.sqrt(((data - mmean) * (data - mmean) * w).sum(axis) *
                            (V1 / (V1 * V1 - V2)))
        #sigma = w.sum()/((w.sum())**2-(w**2).sum())*(w*(data-mean)**2).sum()
        I1=(w*errors).sum(axis)/w.sum(axis)
        sint=np.max([I1/np.sqrt(N),sigma/np.sqrt(N)])     
    else:
        # unweighted sample variance
        sigma = np.sqrt(((data * data).sum(axis) - N * mean * mean) /
                           (N - 1))
        sint=sigma/np.sqrt(N)
    return mean, sigma, sint
...
def sigmaclip(data, errors=None, niter=0, siglow=3., sighigh=3.,
    ...
    indices = np.ones(len(data.ravel()), dtype=np.bool).reshape(
        data.shape)
    ...
    for i in range(nniter):
        ...
        if use_median:
            ...
            mean = None
        mean, sigma, sint = calcsigma(newdata, errors=newerrors)
